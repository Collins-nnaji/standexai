  ...
  return (
    <section className="relative py-24 bg-zinc-900 overflow-hidden">
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-6 lg:px-12">
        ...
        <div className="grid md:grid-cols-3 gap-0 border border-white/10 rounded-[48px] overflow-hidden bg-white/[0.02] backdrop-blur-3xl">
          {metrics.map((metric, i) => (
            <motion.div
              ...
              className={cn(
                ...
                i < metrics.length - 1 && "md:border-r border-b md:border-b-0"
              )}
            >
              <div className="flex flex-col h-full uppercase tracking-[0.25em]">
                <div className="mb-8 flex items-center justify-between">
                   <span className="text-[9px] font-black text-zinc-500 group-hover:text-[#7C5CFC] transition-colors">{metric.category}</span>
                ...
                <div className="mt-auto pt-8 border-t border-white/5 flex items-end justify-between">
                   <div className="flex flex-col">
                      <span className={cn("text-4xl lg:text-6xl font-black tracking-tighter transition-transform duration-500 group-hover:scale-105 origin-left", metric.color)}>
                        {metric.value}
                      </span>
                      <span className="mt-2 text-[8px] font-black text-zinc-600 uppercase tracking-widest">{metric.label.split(" ")[1] ?? "Metric"}</span>
                   </div>
