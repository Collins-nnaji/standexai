...
const FeatureCard = ({ icon, title, description, delay, isVisible }: { icon: React.ReactNode; title: string; description: string; delay: number; isVisible: boolean }) => (
  <div 
    className={`flex flex-col items-center text-center bg-white rounded-2xl p-8 border border-zinc-200 transition-all duration-700 transform hover:-translate-y-1 hover:shadow-lg hover:shadow-[#7C5CFC25] ${
  ...
  >
    ...
    <h3 className="text-xl font-bold text-zinc-900 tracking-tight mb-4">{title}</h3>
  ...
  return (
    <div className="w-full bg-[#f6fbf8] py-16">
      <div ref={sectionRef} className="max-w-7xl mx-auto px-4">
        <h1 
          className={`text-4xl md:text-5xl font-bold tracking-tight text-center text-zinc-900 mb-4 transform transition-all duration-700 ${
        ...
        <p 
          className={`text-center text-zinc-600 mb-12 max-w-4xl mx-auto transform transition-all duration-700 delay-100 ${
            animationStarted ? 'translate-y-0 opacity-100' : 'translate-y-10 opacity-0'
          }`}
        >
