...
const PowerPagesSolution = () => {
  ...
  const serviceCards = [
    {
      icon: (
        <svg className="w-8 h-8 text-zinc-900" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg">
    ...
    <div className="py-16 px-4 w-full bg-[#f6fbf8]">
      <div ref={sectionRef} className="max-w-7xl mx-auto">
        ...
        <div className="text-center mb-12">
          ...
          <h2 
            className={`text-4xl font-black text-zinc-900 tracking-tight max-w-3xl mx-auto transform transition-all duration-700 delay-100 ${
              animationStarted ? 'translate-y-0 opacity-100' : 'translate-y-10 opacity-0'
            }`}
          >
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          {serviceCards.map((card, index) => (
            <div 
              key={index} 
              className={`bg-white border border-zinc-200 rounded-lg shadow-md p-6 flex flex-col h-full transform transition-all duration-700 hover:-translate-y-1 hover:shadow-lg hover:border-[#7C5CFC] ${
                animationStarted ? 'translate-y-0 opacity-100' : 'translate-y-10 opacity-0'
              }`}
              style={{ transitionDelay: `${200 + index * 100}ms` }}
            >
              <div 
                className="bg-[#7C5CFC] rounded-full w-16 h-16 flex items-center justify-center mb-4"
                style={{ 
