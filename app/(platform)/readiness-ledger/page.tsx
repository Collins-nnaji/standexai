...
const PLACEHOLDER_SCANS: ReadinessScan[] = [
  { scanId: "demo-001", dataset: "Payments", readinessScore: 47, tier: "critical", flagCount: 5, cachedAt: new Date(Date.now() - 1000 * 60 * 12).toISOString() },
  { scanId: "demo-002", dataset: "CRM", readinessScore: 87, tier: "moderate", flagCount: 1, cachedAt: new Date(Date.now() - 1000 * 60 * 34).toISOString() },
  { scanId: "demo-003", dataset: "Finance", readinessScore: 94, tier: "excellent", flagCount: 0, cachedAt: new Date(Date.now() - 1000 * 60 * 55).toISOString() },
  { scanId: "demo-004", dataset: "Ops", readinessScore: 61, tier: "high", flagCount: 3, cachedAt: new Date(Date.now() - 1000 * 60 * 82).toISOString() },
  { scanId: "demo-005", dataset: "Support", readinessScore: 78, tier: "moderate", flagCount: 2, cachedAt: new Date(Date.now() - 1000 * 60 * 120).toISOString() },
];
...
export default function ReadinessLedgerPage() {
  const router = useRouter();
  const [scans, setScans] = useState<ReadinessScan[]>([]);
  ...
  const fetchScans = async () => {
    ...
    try {
      ...
      const data = await res.json() as { scans?: ReadinessScan[]; error?: string };
  ...
  const avgScore = scans.length > 0
    ? Math.round(scans.reduce((acc, s) => acc + s.readinessScore, 0) / scans.length)
    : 0;
      ...
      <main className="flex-1 overflow-auto p-8 lg:p-12 max-w-[1500px] mx-auto w-full print:p-0">
              ...
              <table className="w-full text-left">
                <thead>
                  <tr className="bg-[#F9FAFB] border-b border-zinc-100">
                    <th className="px-8 py-5 text-[10px] font-black uppercase tracking-[0.3em] text-zinc-400">Dataset</th>
                    <th className="px-8 py-5 text-[10px] font-black uppercase tracking-[0.3em] text-zinc-400">Readiness Score</th>
                    ...
                    <tr>
                      <td colSpan={6} className="px-8 py-16 text-center">
                        <p className="text-xs font-bold text-zinc-300 uppercase tracking-widest italic">
                          No readiness scans in ledger. Run your first diagnostic.
                        </p>
                      </td>
                    </tr>
                  ) : scans.map((scan) => {
                      ...
                      <tr key={scan.scanId} className="hover:bg-zinc-50/50 transition-colors group">
                        ...
                        <td className="px-8 py-7">
                          <div className="flex flex-col gap-1.5">
                            <span className={`text-2xl font-black ${style.score}`}>{scan.readinessScore}</span>
                            <div className="w-24 h-1.5 bg-zinc-100 rounded-full overflow-hidden">
                              <div className={`h-full ${style.bar} transition-all duration-700`} style={{ width: `${scan.readinessScore}%` }} />
                            </div>
                          </div>
                        </td>
          ...
          <div className="relative z-10 flex flex-col md:flex-row items-start gap-10 justify-between">
            <div>
              ...
              <p className="text-base text-zinc-400 font-medium leading-relaxed max-w-lg">
                Readiness Certificates are verifiable credentials — third-party proof that a dataset meets AI readiness standards. Usable in compliance filings, investor materials, and regulatory submissions.
              </p>
              <div className="flex flex-wrap gap-3 mt-6">
                {["EU AI Act 2026", "FTC Compliance", "SEC Disclosure", "PII Controls"].map((tag) => (
        ...
        <div className="mt-16 text-center pb-12">
          <p className="text-[10px] font-black uppercase tracking-[0.8em] text-zinc-200">
            StandexAI Data Readiness Protocol v1.0 // AI Data Analysis Infrastructure
          </p>
        </div>
